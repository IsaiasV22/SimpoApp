...
CREATE TABLE IF NOT EXISTS `srse`.`usuario` (
  ...
  `notifica_actividad` INT NOT NULL DEFAULT 1,
...
-- -----------------------------------------------------
-- Table `srse`.`recordatorio`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `srse`.`recordatorio` (
  `PK_recordatorio` INT NOT NULL AUTO_INCREMENT,
  `recordatorio` TEXT(400) NOT NULL,
  `FK_actividad` INT NOT NULL,
  PRIMARY KEY (`PK_recordatorio`, `FK_actividad`),
...
CREATE INDEX `FK_actividad_idx` ON `srse`.`recordatorio` (`FK_actividad` ASC) VISIBLE;
...
CREATE TABLE IF NOT EXISTS `srse`.`asistencia_actividad_evento` (
  `FK_actividad_a` INT NOT NULL,
  `FK_usuario_a` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`FK_actividad_a`, `FK_usuario_a`),
  CONSTRAINT `fk_actividad_asist`
    FOREIGN KEY (`FK_actividad_a`)
    REFERENCES `srse`.`actividad` (`PK_actividad`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_usuario_asist`
    FOREIGN KEY (`FK_usuario_a`)
    REFERENCES `srse`.`usuario` (`PK_nombre_usuario`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE INDEX `fk_usuario_idx` ON `srse`.`asistencia_actividad_evento` (`FK_usuario_a` ASC) VISIBLE;
