...
CREATE TABLE IF NOT EXISTS `srse`.`simpo_app_notificacion` (
  `PK_simpo_app_notificacion` INT NOT NULL AUTO_INCREMENT,
  `endpoint` VARCHAR(255) NOT NULL,
  `tiempo_expiracion` TIMESTAMP NULL,
  `p256dh` VARCHAR(255) NOT NULL,
  `autenticador` VARCHAR(255) NOT NULL,
  PRIMARY KEY (`PK_simpo_app_notificacion`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `srse`.`usuario_notificacion_simpo_app` (
  ...
  `FK_simpo_app_notificacion` INT NOT NULL,
  ...
  CONSTRAINT `FK_simpo_app_notificacion`
    ...
    REFERENCES `srse`.`simpo_app_notificacion` (`PK_simpo_app_notificacion`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
