...
const useGlobalState = create((set) => {
  ...
  const suscribedState = typeof window !== 'undefined' && localStorage.getItem('suscribed');
  const initialState = {
    ...
    setSuscribed: (suscribed) => {
      // Update the user state and store it in localStorage
      set({ suscribed });
      typeof window !== 'undefined' && localStorage.setItem('suscribed', JSON.stringify(suscribed));
    },
  };
