...
export default function ActividadesFilter({ elementId, filterFunction}) {
  ...
  useEffect(() => {
    ...
    await handleActividades();
    console.log("Actividades before attachment : ", actividades);
    await attachPonentetoActividades();
    };
    fetchData();
    console.log("Actividades after attachment : ", actividades);
  }, [filterFunction]);
  ...
  async function handleEstadoEstatus(PK_actividad, estatus) {
    ...
    async function attachPonentetoActividades () {
        setActividades( actividades.reduce(async (acum, curr) => 
             [...acum, {...curr, ponente: await handlePonente(curr.PK_actividad)}], []));
        }
                ...
                <div key={element.PK_actividad} className={`col-12 mb-4`}>
                  ...
                  >
                    <div className="card-body position-relative">
                      ...
                      <p className="card-text">
                      <PonenteActividadesCard actividadIdP={element.PK_actividad}/>
                      </p>
