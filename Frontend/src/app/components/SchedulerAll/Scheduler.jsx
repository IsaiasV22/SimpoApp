...
function Scheduler(props) {
  ...
  const getDayRows = () => {
    ...
    let data = [];
    let dayStartHour = startOfDay(selectedDay);
    dayStartHour.setHours(7, 0, 0); // Inicio a las 7 AM
    let ongoingEvent = null; // Para rastrear un evento en curso

    for (let i = 12; i <= HOURS; i++) {
        let id = `line_${i}`;
        let label = format(dayStartHour, 'HH:mm');
        let obj = { id: id, label: label, days: [], eventHeight: 1 };
        ...
        if (!ongoingEvent) {
            ...
            if (matchedEvents.length) {
                ongoingEvent = matchedEvents[0];
            }
        } 

        if (ongoingEvent) {
            let eventEndHour = parse(ongoingEvent?.endHour, 'HH:mm', new Date()).getHours();
            let currentHour = parse(label, 'HH:mm', new Date()).getHours();
            if (currentHour > eventEndHour) {
                ongoingEvent = null; // El evento en curso ha terminado
            }
        }
        ...
        obj.days.push({
            ...
            date: column?.date,
            data: ongoingEvent ? [ongoingEvent] : [],
            height: obj.eventHeight
        });
